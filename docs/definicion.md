...
- **El paciente (externo) — destino:** un RAG **real**, corriendo como su propio servicio (su propia ventana), al que Ratchet se conecta **por afuera** por el adaptador `RagPatientPort`. *(Hoy: stub in-process con `generate()` stubbeado; el paciente externo es net-new.)* El **transporte** de conexión —**MCP** (estándar 2026) vs HTTP— es **dirección intencionada, a fijar en un ADR** tras el spike, no un hecho ya adoptado. Demo sobre **normas contables públicas (NIIF/NIA)**. Que no esté metido dentro de Ratchet mata la crítica del "paciente amañado".
